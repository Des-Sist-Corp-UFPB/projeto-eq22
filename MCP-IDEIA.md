# Ideia de Servidor MCP — EQ22

**Domínio:** Escrita colaborativa de livros (níveis de acesso)  
**Data:** 2026-07-01
...
## Servidor proposto: `iwrite-mcp`

### Tools sugeridas

- `criar_capitulo(livroId, titulo)` — novo capítulo
- `sugerir_continuacao(capituloId)` — IA continua o texto
- `revisar_texto(capituloId)` — revisão/sugestões
- `exportar_livro(livroId, formato)` — exporta
- `gerenciar_colaborador(livroId, userId, nivel)` — permissões

### Resources (somente leitura)

- conteúdo do livro/capítulos como resource (respeitando o nível de acesso)

### Exemplos de uso com um LLM

- "Continue o capítulo 4 no mesmo tom e depois revise a gramática."
...
@Service
public class IwriteTools {

    private final SeuService seuService;   // injete seus services/repositories

    public IwriteTools(SeuService seuService) { this.seuService = seuService; }

    @Tool(description = "novo capítulo")
    public Object criar_capitulo(/* params */) {
